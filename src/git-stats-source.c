...
const char SPACES[SPACES_SIZE] = 
  {[0 ... SPACES_SIZE - 2] = ' ', [SPACES_SIZE-1] = '\0'};
...
void wait_sig_time(struct git_thread_ctx* thread_data){
  ...
  (void)pthread_cond_timedwait(&(thread_data->sleep_interrupt),
                               &(thread_data->data_mutex), &spec);   
...
void* libgit2_thread(void* data){
  ...
  while(!thread_data->exit){
    ...
    if(thread_data->update_request){
      ...
      (void)git_repository_open(&(thread_data->repo), 
                                thread_data->repository_path);
    }
    
      git_diff* diff = NULL;
      git_diff_stats* stats = NULL;
      (void)git_diff_index_to_workdir(&diff, thread_data->repo, NULL, 
                                      &(thread_data->opts));
      ...
      #ifdef TEST_MODE
        info("[git-stats] [TEST] %d %d", thread_data->insertions, 
             thread_data->deletions);
      #endif
...
static void git_stats_update(void *data, obs_data_t *settings)
{
  ...
  ctx->overload_char = extract_unicode(
  ...
  obs_data_set_bool(insertion_settings, "antialiasing", 
                    obs_data_get_bool(settings, "antialiasing"));
  obs_data_set_bool(insertion_settings, "outline", 
                    obs_data_get_bool(settings, "outline"));
  obs_data_set_bool(insertion_settings, "drop_shadow", 
                    obs_data_get_bool(settings, "drop_shadow"));
  obs_data_set_int(insertion_settings, "color1", 
                   obs_data_get_int(settings, "insertion_color1"));
  obs_data_set_int(insertion_settings, "color2", 
                   obs_data_get_int(settings, "insertion_color2"));
  ...
  obs_data_set_bool(deletion_settings, "antialiasing", 
                    obs_data_get_bool(settings, "antialiasing"));
  obs_data_set_bool(deletion_settings, "outline", 
                    obs_data_get_bool(settings, "outline"));
  obs_data_set_bool(deletion_settings, "drop_shadow", 
                    obs_data_get_bool(settings, "drop_shadow"));
  obs_data_set_int(deletion_settings, "color1", 
                   obs_data_get_int(settings, "deletion_color1"));
  obs_data_set_int(deletion_settings, "color2", 
  ...
  obs_data_get_bool(settings, "untracked") ? 
    git_opts.flags = (GIT_DIFF_SHOW_UNTRACKED_CONTENT | 
...
{ 
  ...
  (void)pthread_create(&(ctx->git_thread), NULL, libgit2_thread, 
                       ctx->thread_ctx);
...
static obs_properties_t *git_stats_properties(void *data)
{
  ...
  (void)obs_properties_add_text(general_properties, "overload_info", 
                                ...
                                OBS_TEXT_INFO);
  (void)obs_properties_add_bool(general_properties, "untracked", 
                                obs_module_text("Include Untracked Files"));
...
static void git_stats_tick(void *data, float seconds){
  ...
  if(ctx->thread_ctx->new_val_status){
    obs_data_t* insertion_settings = 
      obs_source_get_settings(ctx->ft2_insertion);
    obs_data_t* deletion_settings = 
      obs_source_get_settings(ctx->ft2_deletion);
    ...
    pthread_mutex_unlock(&(ctx->thread_ctx->data_mutex));
    num_overload_insertion > MAX_DIGITS ? num_overload_insertion = MAX_DIGITS 
      : num_overload_insertion;
    ...
    (void)snprintf(insertion_value_string+1, MAX_DIGITS+1, "%d", 
                   truncated_insertion_val); 
    pad_string_left(insertion_value_string, MAX_DIGITS+1);   
    sprintf(text_buffer, "%s\n%s", insertion_overload_string, 
            insertion_value_string);
    ...
    pthread_mutex_unlock(&(ctx->thread_ctx->data_mutex));
    num_overload_deletion > MAX_DIGITS ? num_overload_deletion = MAX_DIGITS 
      : num_overload_deletion;
    
    char deletion_overload_string[SPACES_SIZE + 1 + ((MAX_DIGITS*4)+2)];
    (void)memset(deletion_overload_string, '\0', 
                 sizeof(deletion_overload_string));
    ...
    (void)snprintf(deletion_value_string+SPACES_SIZE, MAX_DIGITS+1, 
                   "%d", truncated_deletion_val); 
    pad_string_right(deletion_value_string+SPACES_SIZE, MAX_DIGITS+1);

    sprintf(text_buffer, "%s\n%s", deletion_overload_string, 
            deletion_value_string);

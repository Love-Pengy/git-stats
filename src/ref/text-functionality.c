...
void set_up_vertex_buffer(struct ft2_source* srcdata) {
    ...
    eos_check:;
        if (x + word_width > srcdata->custom_width) {
            if (space_pos != 0) srcdata->text[space_pos] = L'\n';
            x = 0;
        }
        ...
        word_width = 0;
        if (srcdata->text[i] == L'\n') x = 0;
        if (srcdata->text[i] == L' ') space_pos = i;
    next_char:;
...
void fill_vertex_buffer(struct ft2_source* srcdata) {
    ...
    draw_glyph:;
        ...
        glyph_index = FT_Get_Char_Index(srcdata->font_face, srcdata->text[i]);
        if (src_glyph == NULL) goto skip_glyph;
    ...
    skip_custom_width:;
        ...
        set_v3_rect(
            vdata->points + (cur_glyph * 6), (float)dx + (float)src_glyph->xoff,
            (float)dy - (float)src_glyph->yoff, (float)src_glyph->w,
            (float)src_glyph->h);
        set_v2_uv(
            tvarray + (cur_glyph * 6), src_glyph->u, src_glyph->v,
            src_glyph->u2, src_glyph->v2);
        set_rect_colors2(
            col + (cur_glyph * 6), srcdata->color[0], srcdata->color[1]);
        dx += src_glyph->xadv;
...
void load_glyph(
    struct ft2_source* srcdata, const FT_UInt glyph_index,
...
struct glyph_info* init_glyph(
    FT_GlyphSlot slot, const uint32_t dx, const uint32_t dy, const uint32_t g_w,
    const uint32_t g_h) {
...
uint8_t get_pixel_value(
    const unsigned char* buf_row, FT_Render_Mode render_mode,
    const uint32_t x) {
...
void rasterize(
    struct ft2_source* srcdata, FT_GlyphSlot slot,
    const FT_Render_Mode render_mode, const uint32_t dx, const uint32_t dy) {
    /**
    ...
    for (uint32_t y = 0; y < slot->bitmap.rows; y++) {
        ...
        for (uint32_t x = 0; x < slot->bitmap.width; x++) {
            ...
            const uint8_t pixel_value = get_pixel_value(
                &slot->bitmap.buffer[row_start], render_mode, x);
...
void cache_glyphs(struct ft2_source* srcdata, wchar_t* cache_glyphs) {
    if (!srcdata->font_face || !cache_glyphs) return;
    ...
    if (cached_glyphs > 0) {
        ...
        srcdata->tex = gs_texture_create(
            texbuf_w, texbuf_h, GS_A8, 1, (const uint8_t**)&srcdata->texbuf, 0);
...
void load_text_from_file(struct ft2_source* srcdata, const char* filename) {
    ...
    os_utf8_to_wcs(
        tmp_read, strlen(tmp_read), srcdata->text, (strlen(tmp_read) + 1));
...
void read_from_end(struct ft2_source* srcdata, const char* filename) {
    ...
    if (bytes_read == 2 && value == 0xFEFF) utf16 = true;
    ...
    if (cur_pos != 0) cur_pos += (utf16) ? 2 : 1;

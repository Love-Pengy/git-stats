...
static inline bool write_data(
    struct serializer* s, const void* data, size_t size) {
...
#define do_write(var)             \
    ...
    for (size_t i = 0; i < font_list.num; i++) {
        ...
        if (info->num_sizes) {
            success =
                write_data(&s, info->sizes, sizeof(int) * info->num_sizes);
            if (!success) break;
        }
...
static void add_font_path(
    FT_Face face, FT_Long idx, const char* family_in, const char* style_in,
    const char* path) {
    struct dstr face_and_style = {0};
    struct font_path_info info;

    if (!family_in || !path) return;
...
void build_font_path_info(FT_Face face, FT_Long idx, const char* path) {
    ...
    for (FT_UInt i = 0; i < num_names; i++) {
        ...
        if (ret != 0 || name.name_id != TT_NAME_ID_FONT_FAMILY) continue;
        ...
        if (family) da_push_back(family_names, &family);
    }
    ...
    for (size_t i = 0; i < family_names.num; i++) {
        ...
        if (i > 0) bfree(family_names.array[i]);
    }
...
const char* get_font_path(
    const char* family, uint16_t size, const char* style, uint32_t flags,
    FT_Long* idx) {
    ...
    for (size_t i = 0; i < font_list.num; i++) {
        ...
        if (rating < info->face_len) continue;

...
void addElementHT(hashTable *table, char *key, untrackedFile value)
{
	...
	if ((*table) == NULL) {
		...
		while ((*table)->files[hashKey(key, numCollisions,
					       (*table)->size)] != NULL) {
		...
		if (notFull) {
			(*table)->files[hashKey(key, numCollisions,
						(*table)->size)] =
			...
			strcpy((*table)->keys[hashKey(key, numCollisions,
						      (*table)->size)],
			       key);
		}
	}
...
void removeElementHT(hashTable *table, char *key)
{
	...
	if (((*table) != NULL) && keyExists((*table), key)) {
		while (1) {
			if ((!((*table)->keys[hashKey(key, numCollisions,
						      (*table)->size)] ==
			       NULL)) &&
			    !strcmp((*table)->keys[hashKey(key, numCollisions,
							   (*table)->size)],
				    key)) {
				break;
			}
		...
		free((*table)->keys[hashKey(key, numCollisions,
					    (*table)->size)]);
		freeUntrackedFile(&((*table)->files[hashKey(key, numCollisions,
							    (*table)->size)]));
...
untrackedFile findValueHT(hashTable table, char *key)
{
	...
	while (table->files[hashKey(key, numCollisions, table->size)] != NULL) {
		if ((!strcmp(key, table->keys[hashKey(key, numCollisions,
						      table->size)]))) {
			found = true;
			break;
		} else if ((table->keys[numCollisions] != NULL) &&
